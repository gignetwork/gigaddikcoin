...
#include "qt/gigaddik/privacywidget.h"
#include "qt/gigaddik/forms/ui_privacywidget.h"
#include "qt/gigaddik/qtutils.h"
#include "guiutil.h"
#include "qt/gigaddik/denomgenerationdialog.h"
#include "qt/gigaddik/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(GigAddikGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zGIG anonymizes your GIG by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zGIG or convert back to GIG"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 GIG ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zGIG");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zGIG");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zGIG");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zGIG");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zGIG");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zGIG");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zGIG");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zGIG");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zGIG Movements"));
    ...
    // Only Convert to GIG enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzGIG->setTitleClassAndText("btn-title-grey", "Total 0 zGIG");
    ui->btnTotalzGIG->setSubTitleClassAndText("text-subtitle", "Show denominations of zGIG owned.");
    ui->btnTotalzGIG->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select GIG outputs to mint into zGIG.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zGIG");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzGIG, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zGIG");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of GIG to mint into zGIG"));
    }else{
        btnText = tr("Convert back to GIG");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zGIG to convert back into GIG"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzGIG->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzGIG->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zGIG is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zGIG, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zGIG").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zGIG minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zGIG converted back to GIG"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any GIG to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGIG </b>";
    ...
    ui->btnTotalzGIG->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
