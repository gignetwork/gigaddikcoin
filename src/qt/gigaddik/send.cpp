...
#include "qt/gigaddik/send.h"
#include "qt/gigaddik/forms/ui_send.h"
#include "qt/gigaddik/addnewcontactdialog.h"
#include "qt/gigaddik/qtutils.h"
#include "qt/gigaddik/sendchangeaddressdialog.h"
#include "qt/gigaddik/optionbutton.h"
#include "qt/gigaddik/sendconfirmdialog.h"
#include "qt/gigaddik/myaddressrow.h"
#include "qt/gigaddik/guitransactionsutils.h"
...
SendWidget::SendWidget(GigAddikGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (GIG)"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("GigAddik address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 GIG");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_GIG : AskPassphraseDialog::Context::Send_zGIG, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zGIG is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zGIG selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zGIG transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZGIG_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zGIG require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zGIG transaction
        ...
        emit message("zGIG transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isGIG){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zGIG in your balance to select."));
        }
    }
}
...
void SendWidget::onGIGSelected(bool _isGIG){
    isGIG = _isGIG;
    setCssProperty(coinIcon, _isGIG ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
