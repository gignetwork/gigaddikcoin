...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::GIG);
    ...
    if (settings.contains("nAnonymizeGigAddikAmount") || reset)
        SoftSetArg("-anonymizegigaddikamount", settings.value("nAnonymizeGigAddikAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in gigaddik.cpp during shotdown to also remove the window positions
