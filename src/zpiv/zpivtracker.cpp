...
CzGIGTracker::CzGIGTracker(std::string strWalletFile)
{
...
CzGIGTracker::~CzGIGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzGIGTracker::Init()
{
...
bool CzGIGTracker::Archive(CMintMeta& meta)
{
...
bool CzGIGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzGIGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzGIGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzGIGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzGIGTracker::GetSerialHashes()
{
...
CAmount CzGIGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzGIGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzGIGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzGIGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzGIGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzGIGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzGIGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzGIGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzGIGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzGIGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzGIGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzGIGWallet* zGIGWallet)
{
    bool iszGIGWalletInitialized = (NULL != zGIGWallet);
    CMintMeta meta;
    ...
    if (! iszGIGWalletInitialized)
        zGIGWallet = new CzGIGWallet(strWalletFile);
    meta.isSeedCorrect = zGIGWallet->CheckSeed(dMint);
    if (! iszGIGWalletInitialized)
        delete zGIGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzGIGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGIGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzGIGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzGIGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzGIGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzGIGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzGIGWallet* zGIGWallet = new CzGIGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zGIGWallet);
        }
        delete zGIGWallet;
