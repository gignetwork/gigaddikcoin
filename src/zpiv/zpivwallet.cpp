// Copyright (c) 2017-2019-2024 The GigAddik Coin developers
...
CzGIGWallet::CzGIGWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGIGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGIGSeed_deprecated()) {
                    LogPrintf("%s: Updated zGIG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzGIGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGIGCount(nCountLastUsed);
    else if (!walletdb.ReadZGIGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGIGWallet::Lock()
{
    seedMaster = 0;
}

void CzGIGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGIGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGIG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGIGWallet::LoadMintPoolFromDB()
{
...
void CzGIGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGIGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGIGWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzGIGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGIG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzGIGWallet::SeedToZGIG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGIGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGIGWallet::GenerateDeterministicZGIG(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGIGWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGIGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzGIGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
