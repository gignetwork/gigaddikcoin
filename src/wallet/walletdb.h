...
public:
    ...
    bool WriteZGIGSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZGIGSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZGIGSeed_deprecated(uint256& seed);
    bool EraseZGIGSeed();
    bool EraseZGIGSeed_deprecated();

    bool WriteZGIGCount(const uint32_t& nCount);
    bool ReadZGIGCount(uint32_t& nCount);
