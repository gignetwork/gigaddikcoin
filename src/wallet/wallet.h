...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 GIG at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zGIG send
enum ZerocoinSpendStatus {
    ZGIG_SPEND_OKAY = 0,                            // No error
    ZGIG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGIG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGIG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGIG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGIG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGIG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGIG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGIG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGIG_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGIG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGIG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGIG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGIG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGIG_SPENT_USED_ZGIG = 14,                      // Coin has already been spend
    ZGIG_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZGIG_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZGIGOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzGIGTracker> zpivTracker;
    ...
    void setZWallet(CzGIGWallet* zwallet);
    CzGIGWallet* getZWallet();
    ...
    /// Get 10000 GIG output and keys which can be used for the Masternode
    ...
    /** zGIG reset */
    boost::signals2::signal<void()> NotifyzGIGReset;
