...
*Setup instructions for a gitian build of GigAddik Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the GigAddik
...
to the GigAddik GitHub release page.
...
- [Building GigAddik Core](#building-gigaddik-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/gigaddik-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/gigaddik-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/gigaddik-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/gigaddik-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/gigaddik-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/gigaddik-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/gigaddik/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@addik.co) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/gigaddik-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/gigaddik-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/gigaddik-osx-*-build.assert
```
